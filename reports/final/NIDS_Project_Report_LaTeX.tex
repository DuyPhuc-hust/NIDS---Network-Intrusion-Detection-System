...
The following two matrices are therefore included to show how the strongest benchmark model and the baseline model behave on the same test split. KNN is still compared through the overall result table and per-class F1 table. A full KNN confusion matrix is less practical to include because KNN prediction over the full 565,713-flow test set is computationally expensive and does not add much interpretation beyond the per-class F1 scores already shown.

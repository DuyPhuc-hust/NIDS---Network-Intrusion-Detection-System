...
\subsection{Confusion Matrix Selection}

The report includes the XGBoost confusion matrix because XGBoost is the main model used to demonstrate the effect of imbalance handling and PCAP-oriented inference. Adding raw confusion matrices for all four models is possible, but it would make the report much longer without adding much new interpretation. The per-class F1 table already compares all models class by class, while the XGBoost confusion matrix provides detailed error analysis for the model most relevant to the imbalance-handling objective.

If the report requires an appendix, the next useful addition would be confusion matrices for Random Forest without imbalance handling and XGBoost without imbalance handling. These two matrices would directly compare the strongest internal benchmark against the model that benefits most from imbalance handling.
